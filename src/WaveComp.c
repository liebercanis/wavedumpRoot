...
{
    return BoardInfo.FamilyCode == CAEN_DGTZ_XX761_FAMILY_CODE ||
...
{
    return BoardInfo.FamilyCode == CAEN_DGTZ_XX751_FAMILY_CODE ||
           BoardInfo.FamilyCode == CAEN_DGTZ_XX730_FAMILY_CODE ||
           BoardInfo.FamilyCode == CAEN_DGTZ_XX725_FAMILY_CODE;
}
...
int Set_calibrated_DCO(int handle, int ch, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)
{
    ...
    if (WDcfg->PulsePolarity[ch] == CAEN_DGTZ_PulsePolarityPositive)
    {
        ...
        if (WDcfg->DCoffset[ch] > 65535)
            WDcfg->DCoffset[ch] = 65535;
        if (WDcfg->DCoffset[ch] < 0)
            WDcfg->DCoffset[ch] = 0;
    }
    else if (WDcfg->PulsePolarity[ch] == CAEN_DGTZ_PulsePolarityNegative)
    {
...
void CheckKeyboardCommands(int handle, WaveDumpRun_t *WDrun, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)
{
    ...
    if ((c < '9') && (c >= '0'))
    {
        ...
        if ((BoardInfo.FamilyCode == CAEN_DGTZ_XX740_FAMILY_CODE) || (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE))
        {
            if ((BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) && (WDcfg->FastTriggerEnabled == 0) && (ch == 8))
            ...
            else if (WDrun->ChannelPlotMask & (1 << ch))
    ...
    else
    {
        ...
        case 'W':
            ...
            break;
        case 'X':
            WDrun->ContinuousRootWrite ^= 1;
            if (WDrun->ContinuousRootWrite)
                printf("Continuous writing to root file is enabled\n");
            else
                printf("Continuous writing to root file is disabled\n");
            break;
        case 's':
            if (WDrun->AcqRun == 0)
            {
        ...
        case ' ':
            ...
            printf("  [X]   Enable/Disable continuous writing to Root file\n");
...
int WriteOutputFiles(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event)
{
    ...
    // open output file
    if (!WDrun->foutRoot)
    { // if no root file open, then open a new one as run-mm-dd-yyyy-file#.root
        struct tm *timeinfo;
        timeinfo = localtime(&rawtime);
        strftime(dateTag, 11, "%m_%d_%Y", timeinfo);
        TString fullname = (Form("rootData/run-%s-file.root", dateTag));
        WDrun->foutRoot = new TFile(fullname, "update"); // DEF made to update rather than recreate so that it doesn't write over a file already made.
        printf("opened output file %s \n", WDrun->foutRoot->GetName());
        rawRun = new TBRawRun(dateTag);
        rawRun->btree->SetMaxTreeSize(2000000000); // DEF Changed to 2Gb max size
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++)
    {
        ...
        if (WDrun->ContinuousRootWrite)
        {
            ...
            // now calculate mean and rms

            double sum = std::accumulate(wave.begin(), wave.end(), 0);
            double mean = sum / wave.size();

            double var = 0;
            for (int n = 0; n < wave.size(); n++)
            {
                var += pow(double(wave[n]) - double(mean), 2.);
            }
            var /= double(wave.size());
            double rms = sqrt(var);
...
Restart:
    ...
    InterruptTimeout:
        ...
        for (i = 0; i < (int)NumEvents; i++)
        {
            ...
            if (WDrun.ContinuousWrite || WDrun.SingleWrite || WDrun.ContinuousRootWrite)
            {
...
QuitProgram:
    ...
    if (WDrun.foutRoot)
    {
        WDrun.foutRoot->Write();
        WDrun.foutRoot->Close();
    }
